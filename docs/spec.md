...
- **Matrix index log**: each matrix execution writes one `pi-run-YYYYMMDD-HHmmss.log` (matrix start time) into the process cwd (git-ignored, survives the temp-dir cleanup). It mirrors every `[pi]`/`[matrix]`/progress line with an ISO timestamp prefix — console output is unchanged — and adds structured markers: `=== Combo i/N START: <question> × <model> (pi-run-<id>) ===` and a self-contained `=== Combo i/N END: <STATUS> (<duration>s) session: <archived session.jsonl path> ===` (`none (reason)` when the combo crashed before archiving). A `=== SUMMARY: N combos, M failed ===` section listing each failure's transcript path is appended whenever the matrix returns, including an API-failure abort; the END lines alone suffice to count and locate failures (`grep "END: ERROR"`) when the process is killed mid-run. Startup failures (unknown model id, isolation guard) are recorded as `=== MATRIX FAILED: <reason> ===`.
